...
using demoabpapp.blazorservertiered.Permissions;
...
namespace demoabpapp.blazorservertiered.Blazor.Menus;

public class blazorservertieredMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public blazorservertieredMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<blazorservertieredResource>();

        context.Menu.Items.Insert(
            0,
            new ApplicationMenuItem(
                blazorservertieredMenus.Home,
                l["Menu:Home"],
                "/",
                icon: "fas fa-home",
                order: 1
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                blazorservertieredMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(blazorservertieredPermissions.Dashboard.Host)
        );

        //TenantDashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                blazorservertieredMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(blazorservertieredPermissions.Dashboard.Tenant)
        );
